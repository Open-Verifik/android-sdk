...
public class VerifikAppRegisterProcessor extends Processor implements FaceTecFaceScanProcessor {
    ...
    public VerifikAppRegisterProcessor(Context context,
                                       String sessionToken,
                                       String verifikToken,
                                       String project,
                                       String email,
        ...
        //
        FaceTecSessionActivity.createAndLaunchSession(context, VerifikAppRegisterProcessor.this, sessionToken);
    }
    ...
    public void processSessionWhileFaceTecSDKWaits(final FaceTecSessionResult sessionResult, final FaceTecFaceScanResultCallback faceScanResultCallback) {
        ...
        if (sessionResult.getStatus() != FaceTecSessionStatus.SESSION_COMPLETED_SUCCESSFULLY) {
            verifikCallback.appRegisterError("User cancel App Register or there was a connection error");
        ...
        try {
            ...
            parameters.put("type", "onboarding");
            parameters.put("id", project);
            parameters.put("email", email);
            parameters.put("phone", phone);
        } catch (JSONException e) {
            e.printStackTrace();
            verifikCallback.appRegisterError("App register parameters exception");
        ...
        okhttp3.Request request = new okhttp3.Request.Builder()
                .url(VerifikURL.BaseKYC + VerifikURL.AppRegisterKYC)
        ...
        NetworkingHelpers.getApiClient().newCall(request).enqueue(new Callback() {
                ...
                try {
                    ...
                    if(responseJSON.has("code") &&
                       responseJSON.getString("code").equals("NotFound")){
                        verifikCallback.appRegisterError("User not found on Database");
                        faceScanResultCallback.cancel();
                        return;
                    }
                    if(responseJSON.has("code") &&
                            responseJSON.getString("code").equals("Conflict")){
                        verifikCallback.appRegisterError("Biometric validation failed");
                        faceScanResultCallback.cancel();
                        return;
                    }
                    if(responseJSON.has("code") ||
                        ...
                        verifikCallback.appRegisterError(err);
                        faceScanResultCallback.cancel();
                        return;
                    }
                    ...
                    resultToken = responseData.getString("_id");
                    ...
                    if (wasProcessed) {
                        ...
                        verifikCallback.appRegisterSuccessful(resultToken);
                    } else {
                        // CASE:  UNEXPECTED response from API.  Our Sample Code keys off a wasProcessed boolean on the root of the JSON object --> You define your own API contracts with yourself and may choose to do something different here based on the error.
                        verifikCallback.appRegisterError("No authorization granted");
                        faceScanResultCallback.cancel();
                    }
                } catch (JSONException e) {
                    // CASE:  Parsing the response into JSON failed --> You define your own API contracts with yourself and may choose to do something different here based on the error.  Solid server-side code should ensure you don't get to this case.
                    e.printStackTrace();
                    verifikCallback.appRegisterError("There was an error parsing app register resulting data, please contact Verifik Support Team");
            ...
            public void onFailure(@NonNull Call call, IOException e) {
                ...
                verifikCallback.appRegisterError("There was an error parsing app register resulting data 2, please contact Verifik Support Team");
