...
public class PhotoIDMatchProcessor extends Processor implements FaceTecFaceScanProcessor, FaceTecIDScanProcessor {
    ...
    public void processSessionWhileFaceTecSDKWaits(final FaceTecSessionResult sessionResult, final FaceTecFaceScanResultCallback faceScanResultCallback) {
        ...
        if(sessionResult.getStatus() != FaceTecSessionStatus.SESSION_COMPLETED_SUCCESSFULLY) {
            ...
            verifikCallback.photoIDMatchError("User cancel Photo ID Match or there was a connection error - Face Step");
            return;
        }
        ...
        try {
            ...
            verifikCallback.photoIDMatchError("Photo ID Match parameters exception - Face Step");
        }
        ...
        NetworkingHelpers.getApiClient().newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NonNull Call call, @NonNull okhttp3.Response response) throws IOException {
                ...
                try {
                    ...
                    if(wasProcessed) {
                        ...
                        faceScanResultCallback.cancel();
                        verifikCallback.photoIDMatchError("No enrollment executed");
                    }
                }
                catch(JSONException e) {
                    ...
                    faceScanResultCallback.cancel();
                    verifikCallback.photoIDMatchError("There was an error parsing face scan resulting data - Face Step, please contact Verifik Support Team");
                }
            }
            ...
            public void onFailure(@NonNull Call call, @NonNull IOException e) {
                ...
                faceScanResultCallback.cancel();
                verifikCallback.photoIDMatchError("There was an error parsing face scan resulting data 2 - Face Step, please contact Verifik Support Team");
            }
        });
    }
    ...
    public void processIDScanWhileFaceTecSDKWaits(final FaceTecIDScanResult idScanResult, final FaceTecIDScanResultCallback idScanResultCallback) {
        ...
        // In the code in your own App, you can pass around signals, flags, intermediates, and results however you would like.
        //
        //verifikActivity.setLatestIDScanResult(idScanResult);
        ...
        if(idScanResult.getStatus() != FaceTecIDScanStatus.SUCCESS) {
            verifikCallback.photoIDMatchError("User cancel ID Scan or there was a connection error - ID Step");
            NetworkingHelpers.cancelPendingRequests();
        ...
        try {
            ...
            e.printStackTrace();
            verifikCallback.photoIDMatchError("Photo ID Match parameters exception");
            Log.d("FaceTecSDKSampleApp", "Exception raised while attempting to create JSON payload for upload.");
        }
        ...
        //
        okhttp3.Request request = new okhttp3.Request.Builder()
                .url(VerifikURL.Base + VerifikURL.MatchID)
                .header("Content-Type", "application/json")
                .header("X-Device-Key", Config.getShared().DeviceKeyIdentifier)
        ...
        NetworkingHelpers.getApiClient().newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NonNull Call call, @NonNull okhttp3.Response response) throws IOException {
                ...
                try {
                    ...
                    if(responseJSON.has("code") &&
                        ...
                        verifikCallback.photoIDMatchError("User not found on database");
                        idScanResultCallback.cancel();
                        return;
                    }
                    if(responseJSON.has("code") &&
                            responseJSON.getString("code").equals("Conflict")){
                        verifikCallback.photoIDMatchError("Biometric is disabled");
                        idScanResultCallback.cancel();
                        return;
                    }
                    if(responseJSON.has("code") ||
                        ...
                        verifikCallback.photoIDMatchError(err);
                        idScanResultCallback.cancel();
                        return;
                    }
                    ...
                    if(wasProcessed) {
                        ...
                        // CASE:  UNEXPECTED response from API.  Our Sample Code keys off a wasProcessed boolean on the root of the JSON object --> You define your own API contracts with yourself and may choose to do something different here based on the error.
                        verifikCallback.photoIDMatchError("Photo ID Match not executed");
                        idScanResultCallback.cancel();
                    }
                }
                catch(JSONException e) {
                    // CASE:  Parsing the response into JSON failed --> You define your own API contracts with yourself and may choose to do something different here based on the error.  Solid server-side code should ensure you don't get to this case.
                    e.printStackTrace();
                    verifikCallback.photoIDMatchError("There was an error parsing ID scan resulting data - ID Step, please contact Verifik Support Team");
                    Log.d("FaceTecSDKSampleApp", "Exception raised while attempting to parse JSON result.");
                    idScanResultCallback.cancel();
                }
            }
            ...
            public void onFailure(@NonNull Call call, @NonNull IOException e) {
                // CASE:  Network Request itself is erroring --> You define your own API contracts with yourself and may choose to do something different here based on the error.
                verifikCallback.photoIDMatchError("There was an error parsing ID scan resulting data 2 - ID Step, please contact Verifik Support Team");
                Log.d("FaceTecSDKSampleApp", "Exception raised while attempting HTTPS call.");
                idScanResultCallback.cancel();
            }
        });
